...
print("4. Extracting discrete SOX2 and NANOG peaks from BED files...")
def extract_peaks_from_bed(bed_file):
    print(f"   Extracting peaks from {bed_file}...")
    peaks_tree = {} # chrom -> IntervalTree
    
    # Initialize liftover mm39 -> mm9
    converter = ChainFile('mm39ToMm9.over.chain.gz')
    ...
    with gzip.open(bed_file, 'rt') as f:
        for line in f:
            ...
            if len(parts) >= 3:
                ...
                # We need to lift over mm39 coordinates to mm9
                try:
                    start_mm39 = int(parts[1])
                    end_mm39 = int(parts[2])
                    
                    # liftover requires 0-based coordinates, which BED is.
                    start_res = converter[chrom][start_mm39]
                    end_res = converter[chrom][end_mm39]
...
sox2_peaks = extract_peaks_from_bed('GSM8438448_ESC-SOX2-G1_master_peaks.bed.gz')
nanog_peaks = extract_peaks_from_bed('GSM8438436_ESC-NANOG-G1_master_peaks.bed.gz')
